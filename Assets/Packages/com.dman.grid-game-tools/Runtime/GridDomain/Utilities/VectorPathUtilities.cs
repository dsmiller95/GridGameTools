...
public static class VectorPathUtilities
{
    ...
    public static IEnumerable<Vector3Int> PathFrom0(Vector3Int relative)
    {
        ...
        if (relative.y != 0)
        {
            var sign = Math.Sign(relative.y);
            for (int i = 0; i < Math.Abs(relative.y); i++)
            {
        ...
        if (relative.x != 0)
        {
            var sign = Math.Sign(relative.x);
            for (int i = 0; i < Math.Abs(relative.x); i++)
            {
        ...
        if (relative.z != 0)
        {
            var sign = Math.Sign(relative.z);
            for (int i = 0; i < Math.Abs(relative.z); i++)
            {
